import { addCategory, get_with_token, post, post_with_token } from "@/action"
...
async function layout({ params, children }) {
    ...
    const isMyKitchen = await get_with_token(`jwt/isItMyKitchen/${params.id}`)
    const status = isMyKitchen.status;
  return (
      <div>
          <div className='h-16'></div>
          <div className="">
              <div className='grid grid-cols-2 place-content-center m-auto px-5 justify-center gap-6'>
                  <div className=''>
                      {status ?
                          <CategoryTable id={params.id} />
                          ...
                          <div>
                              <form action={formAction} className="grid gap-4">
                                  <div className="grid gap-2">
                                      ...
                                      <input className='flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50' id="name" placeholder="Enter category name" name='category' />
                                  </div>
                                  <div className="grid gap-2">

...
BEGIN
    IF LENGTH(PASSWORD) < 10 THEN
        RAISE_APPLICATION_ERROR(-20003, 'Password must be at least 10 characters long');
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20001, 'Error registering user: '
                                        || SQLERRM);
END;
/
...
FROM
    ...
    DELIVERY_PARTNER
WHERE
    USERS.ID=DELIVERY_PARTNER.USER_ID;

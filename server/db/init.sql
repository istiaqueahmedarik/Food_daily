...
END;
/
CREATE VIEW CHEF_INFO_VIEW("CHEF NAME","SPECIALITY","EXPERIENCE")
AS
SELECT CHEF_NAME,SPECIALITY,EXPERIENCE FROM CHEF

CREATE VIEW KITCHEN_INFO_VIEW("KITCHEN NAME","ADDRESS","CHEF NAME")
AS
SELECT NAME,ADDRESS,C.CHEF_NAME
FROM KITCHEN K,CHEF C
WHERE K.CHEF_ID=C.ID

CREATE VIEW FOOD_INFO_VIEW("FOOD NAME","CATEGORY","PRICE")
AS
SELECT F.NAME,C.NAME,F.PRICE
FROM FOOD F,CATEGORY C
WHERE F.CATEGORY_ID=C.ID

CREATE VIEW FOOD_INGREDIENT_VIEW("FOOD NAME","INGREDIENT","CALORIES")
AS
SELECT F.NAME,I.NAME,I.CALORIES
FROM FOOD F,INGREDIENT I
WHERE F.ID=I.FOOD_ID

CREATE VIEW FOOD_REVIEW_VIEW("FOOD NAME","REVIEW")
AS
SELECT F.NAME,R.REVIEW|| ' BY '||U.FIRST_NAME||' '||U.LAST_NAME AS REVIEW
FROM FOOD F,FOOD_RATING R,USERS U
WHERE F.ID=R.FOOD_ID AND R.USER_ID=U.ID

CREATE VIEW CHEF_COMMISION_VIEW("CHEF NAME","AMOUNT")
AS
SELECT C.CHEF_NAME,M.AMOUNT
FROM CHEF C,CHEF_COMMISION M
WHERE M.CHEF_ID=C.ID

CREATE VIEW QA_OFFICER_VIEW("NAME","QUALIFICATION","DATE OF JOINING")
AS
SELECT U.FIRST_NAME||' '||U.LAST_NAME AS NAME,Q.ACADEMIC_QUALIFICATION,DATE_OF_JOINING 
FROM USERS U,QA_OFFICER Q
WHERE U.ID=Q.USER_ID

CREATE VIEW CERTIFICATION_VIEW("CHEF NAME","CERTIFICATION","EXPIRY DATE")
AS
SELECT C.CHEF_NAME,S.CERTIFICATION,S.EXPIRY_DATE
FROM CHEF C,CERTIFICATION S
WHERE C.ID=S.CHEF_ID

CREATE VIEW KITCHEN_IMAGE_VIEW("KITCHEN NAME","CHEF NAME","IMAGE","DATE ADDED")
AS
SELECT K.NAME ,C.CHEF_NAME ,KI.IMAGE,KI.DATE_ADDED
FROM KITCHEN K,CHEF C,KITCHEN_IMAGES KI
WHERE KI.KITCHEN_ID=K.ID AND K.CHEF_ID=C.ID

CREATE VIEW ORDERS_VIEW("USERS ID","DATE ADDED","SHIPPING ADDRESS","PHONE NUMBER")
AS
SELECT USER_ID,DATE_ADDED,SHIPPING_ADD,SHIPPING_PHONE
FROM ORDERS

CREATE VIEW CART_VIEW("USERS_ID","FOOD_ID","QUANTITY")
AS
SELECT USER_ID,FOOD_ID,QUANTITY
FROM CART
